...
class KeyDisplay(object):
    def __init__(
        ...
        if self.zmq_port is not None:
            print(f'KeyDisplay: connecting to zmq port {self.zmq_port} ...')
            ...
            print("KeyDisplay: no zmq port specified")
...
@click.option("--zmq_port", default=5556, help="ZMQ port, for live input") # here I modified the default value from None to 5556. Otherwise there will be a bug in tcp connection (the sender side does not set up the port number, so the receiver side will not be able to receive the message.)
